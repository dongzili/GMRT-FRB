...
GAIN/DGAIN solution from :measure_gain_dgain.py:
DPHASE solution from :measure_crossdelay.py:
...
def get_args ():
    import argparse
    agp = argparse.ArgumentParser ("make_pacv_qpsr", description="Makes a pacv calibration solution file using quasar/pulsar solutions", epilog="GMRT-FRB polarization pipeline")
    add = agp.add_argument
    add ('-g','--gdg', help='Gain/Dgain solution (output of measure_gain_dgain.py)',required=True, dest='gdg')
    add ('-p','--dphase', help='Dphase solution (output of measure_crossdelay.py)',required=True, dest='dphase')
...
class BasePacvInfo(object):
    ...
    def fill_solution_header(self, gdg_file, dphase_file ):
        """
        Put solution
        ...
        ## filenames
        t_hdr['GDG_FILE'] = (gdg_file, "GDG solution file")
        t_hdr['DP_FILE']  = (dphase_file, "DP solution file")
...
if __name__ == "__main__":
    ...
    ###################################################
    base,_      = os.path.splitext ( os.path.basename ( args.dphase ) )
    base        = base.split('.')[0]
    ofile       = base + ".mycal_qpsr.pacv"
    outfile     = os.path.join ( args.odir, ofile  )
    ###################################################
    ### read solutions
    ###################################################
    gdg     = np.load ( args.gdg )
    dphase  = np.load ( args.dphase )
    ####  take params from dphase
    freq    = np.ma.MaskedArray (gdg['freq'], mask=gdg['freq_mask'])
    bias    = dphase['bias']
    dpi     = dphase['dpi']
    ebias     = dphase['biaserr']
    edpi      = dphase['dpierr']
    dpline      =  np.arctan ( np.tan ( -0.5 * ( bias + (dpi * freq * 1E-3) ) ) )
    dpline_err  =  np.arctan ( np.tan ( -0.5 * ( ebias + (edpi * freq * 1E-3) ) ) )
    ###################################################
    ### making a pacv solution
    ###################################################
    tobs      = at.Time ( gdg['mjd'], format='mjd' )
    dt        = tobs.strftime ("%Y%m%d")
    mjd       = int ( gdg['mjd'] )
    pinfo     = BasePacvInfo ( gdg['mjd'] )
    pinfo.fill_freq_info ( gdg['nchan'].item(), gdg['fbw'].item(), freq )
    ##
    pinfo.fill_source_info ( "XXX", 0.0, 0.0 )
    ...
    data[0, ..., 0]     = gdg['gain']
    data[0, ..., 1]     = gdg['dgain']
    dataerr[0, ..., 0]  = gdg['gainerr']
    dataerr[0, ..., 1]  = gdg['dgainerr']
    data[0, ..., 2]     = dpline
    dataerr[0, ..., 2]  = dpline_err
    ##################################################
    ### pacv fits header
    ###################################################
    ...
    calsol_header   = pinfo.fill_solution_header ( 
        gdg_file    = os.path.basename ( args.gdg ), 
        dphase_file = os.path.basename ( args.dphase ) 
    )
    calsol_columns  = [
